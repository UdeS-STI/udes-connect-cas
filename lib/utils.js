...
function shouldIgnore(req, options) {
  if (options.match && options.match.splice && options.match.length) {
    ...
    if (hasMatch) {
      debug('Matched match rule.', matchedRule, ' Go into CAS authentication.');
      return false;
    }
  ...
  if (options.ignore && options.ignore.splice && options.ignore.length) {
    ...
    if (hasMatchIgnore) {
      debug('Matched ignore rule.', matchedIgnoreRule, ' Go through CAS.');
      return true;
    }
...
function getLastUrl(req, options) {
  ...
  if (req.headers && req.headers['referer']) {
    if (req.headers['referer'].startsWith('https://www.usherbrooke.ca/monacces')) {
      if (req.session && req.session.returnUrl) {
        lastUrl = req.session.returnUrl;
      }
    }
  }

  debug('Get lastUrl: ' + lastUrl);
